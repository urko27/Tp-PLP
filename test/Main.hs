...
testsFold =
  test
    [
      foldExpr                   -- Sumamos 1 a los valores numéricos de la expresión
        (\c -> Const (c+1))
        (\a b -> Rango (a+1) (b+1))
        (\p q -> Suma p q)
        (\p q -> Resta p q)
        (\p q -> Mult p q)
        (\p q -> Div p q)
        (Suma (Const 2.0) (Rango 1 5)) ~?= Suma (Const 3.0) (Rango 2 6),
      foldExpr                 -- "Invertimos" las operaciones, suma <-> resta y mult <-> div
        (\c -> Const c)
        (\a b -> Rango a b)
        (\p q -> Resta p q)
        (\p q -> Suma p q)
        (\p q -> Div p q)
        (\p q -> Mult p q)
        (Div (Resta (Suma (Const 2.0) (Rango 1 5)) (Mult (Const 1.0) (Rango 4 8))) (Const 2.0)) ~?=
